...
def read_data(struct_pdb, file_contacts, file_BB, file_OV, file_rCSU, header_lines, cols, Natoms):
    ...
    for k in range(0, len(dat)):
        tmp = dat[k]
        tmp = [tmp[0:6],tmp[6:11],tmp[11:14],tmp[17:20],tmp[20:22],tmp[22:26],tmp[26:38],tmp[38:46],tmp[46:55],tmp[56:60],tmp[60:67]]
...
def write_files(file_pref, sym_pairs, missAt, indBB, missRes, Natoms, nameAA, go_eps, c6c12):
    ...
    with open(file_pref + '_go-table.itp','w') as f:
    ...
    subprocess.call("echo '#include \"" + file_pref + "_go-table.itp\"' >> go-table.itp ", shell=True)

    # write supplementary file: BB virtual particle definitions for martini.itp
    with open(file_pref + '_go-sites.itp','w') as f:
        f.write('[ atomtypes ] \n')
        ...
        for k in range(0, len(indBB)):
            ...
            f.write(s2print)
    subprocess.call("echo '#include \"" + file_pref + "_go-sites.itp\"' >> go-sites.itp ", shell=True)

    # write supplementary file: exclusions for protein.itp
    with open(file_pref + '_go-excl.itp','w') as f:
        f.write(';[ exclusions ] \n')
        f.write('; OV + symmetric rCSU contacts \n')
        for k in range(0, len(sym_pairs)):
    ...
    with open(file_pref + '_go-harm.itp','w') as f:
...
def main():
    args = user_input()
    Natoms=args.Natoms if args.Natoms!=None else 0
    file_BB, file_OV, file_rCSU, header_lines, seqDist, cols, missRes, missAt, c6c12 = get_settings()

    indBB, nameAA, map_OVrCSU, Natoms = read_data(args.s, args.f, file_BB, file_OV, file_rCSU, header_lines, cols, Natoms)
