...
def colbuilder():
    ...
    parser.add_argument('-mix','--setup_mix', required=False,nargs='+',
                        help='Mix-System setup with different crosslink types. E.g. 70 trivalent and 30 divalent crosslink -mix T:70 D:30.\n  Please use -f_mix to input pdbs in same order',default=None)
    parser.add_argument('-fmix','--files_mix', required=False,nargs='+',
                        help='PDB-files for Mix-System  setup. E.g. -fmix Rat-T.pdb Rat-D.pdb \n -fmix needs same order as -mix.',default=[])
    parser.add_argument('-mutate','--setup_mutate', required=False,nargs='+',
                        help='ratio for crosslinks to be mutated (default: None)',default=None)
